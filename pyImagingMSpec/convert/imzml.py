...
def centroid_imzml(input_filename, output_filename, step=[], apodization=False, w_size=10, min_intensity=1e-5, prevent_duplicate_pixels=False):
    ...
    imzml = ImzMLParser(input_filename)

    mz_dtype = imzml.mzPrecision
    int_dtype = imzml.intensityPrecision
    # Convert coords to index -> kinda hacky
    coords = np.asarray(imzml.coordinates).round(5)
    coords -= np.amin(coords, axis=0)
    if step == []:  # have a guesss
        step = np.array([np.median(np.diff(np.unique(coords[:, i]))) for i in range(3)])
    ...
    if prevent_duplicate_pixels:
        ...
        print np.shape(imzml.coordinates), np.shape(coord_idx)
    ...
    with ImzMLWriter(output_filename, mz_dtype=mz_dtype, intensity_dtype=int_dtype) as imzml:
        ...
        for key in range(np.shape(imzml.coordinates)[0]):
            print key
            if all((prevent_duplicate_pixels, key not in coord_idx)):  # skip duplicate pixels
                continue
            mzs, intensities = imzml.get_spectrum(key)
            if apodization:
...
if __name__ == '__main__':
    centroid_imzml(sys.argv[1], sys.argv[1][:-3] + "_centroid.imzML")